...
class GameAdVideo161View(context: Context) : FrameLayout(context) {
    ...
    private val minSizePx get() = resources.designPx(161)

    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        val resolvedWidth = MeasureSpec.getSize(widthMeasureSpec).coerceAtLeast(minSizePx)
        val resolvedHeight = MeasureSpec.getSize(heightMeasureSpec).coerceAtLeast(minSizePx)
        val side = maxOf(resolvedWidth, resolvedHeight)
        val squareSpec = MeasureSpec.makeMeasureSpec(side, MeasureSpec.EXACTLY)
        super.onMeasure(squareSpec, squareSpec)
    }

    init {
        layoutParams = LayoutParams(resources.designPx(161), resources.designPx(161))
        clipToOutline = true
        background = GradientDrawable().apply {
            setColor(Color.parseColor("#F2F3F5"))
            cornerRadius = resources.designPx(10).toFloat()
        }
    ...
    fun bind(gs: GameSlot, onClick: () -> Unit) {
        ...
        if (videoUrl.isNotEmpty() && videoUrl.isVideoUrl()) {
    ...
    private fun String.isVideoUrl(): Boolean {
        val path = Uri.parse(this).path?.lowercase() ?: return false
        return path.endsWith(".mp4") || path.endsWith(".m3u8") ||
               path.endsWith(".webm") || path.endsWith(".mov") || path.endsWith(".avi")
    }
